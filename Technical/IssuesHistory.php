...
<button style="background: Red; float: right" type="button" class="btn" name="cancel" value="cancel" onClick="window.location='dashboardTechnical.php';" />Exit</button>
  	<h4>Issues Listing</h4>
  </div>
<form style="width: 95%; border-radius: 0px 0px 0px 0px;" id="form1" name="form1" method="post" action="\ITcapstone\Technical\issuesHistory.php">
...
<input type="submit" class="btn" name="button" id="button" value="Search" />
  ...
  &nbsp;&nbsp;&nbsp;
  <a href="\ITcapstone\Technical\issuesHistory.php">reset</a>
...
//Part of the search engine codes
$sql_result = mysql_query ($sql, $connection ) or die ('request "Could not execute SQL query" '.$sql);
...
<?php
    $page_query = "SELECT * FROM reported_issues ORDER BY entryID DESC";
    ...
    if($page > 1)
    {
     echo "<a href='\ITcapstone\Technical\issuesHistory.php?page=1'>First</a>";
     echo "<a href='\ITcapstone\issuesHistory.php?page=".($page - 1)."'><<</a>";
    }
    for($i=$start_loop; $i<=$end_loop; $i++)
    {     
     echo "<a href='\ITcapstone\Technical\issuesHistory.php?page=".$i."'>".$i."</a>";
    }
    if($page <= $end_loop)
    {
     echo "<a href='\ITcapstone\Technical\issuesHistory.php?page=".($page + 1)."'>>></a>";
     echo "<a href='\ITcapstone\Technical\issuesHistory.php?page=".$total_pages."'>Last</a>";
    }
